...
// and writes them to data/shuls.json. Runs inside GitHub Actions (see update-shuls.yml).
// Place this file at: scripts/scrape-godaven.mjs
...
const DAY_WORDS = /\b(sun|mon|tue|wed|thu|fri|shabbos|shabbat|sat|weekday|daily|motzei|erev|rosh chodesh|legal holiday)\w*\b/i;
...
async function scrapeShul(page, id) {
  ...
  // Capture any JSON the app loads — most reliable source if GoDaven changes its layout
  const onResponse = async (resp) => {
    try {
      const ct = resp.headers()["content-type"] || "";
      if (ct.includes("application/json")) {
        const body = await resp.json().catch(() => null);
        if (body) apiPayloads.push({ url: resp.url(), body });
      }
    } catch {}
  };
  ...
  // Give the SPA a moment to finish rendering
  await page.waitForTimeout(4000);

  // Basic info is in the meta description even before JS:
  ...
  for (const line of lines) {
    // Track section headers like "Shacharis", "Mincha", "Shabbos", "Shiurim"
    ...
    if (kind === "shiur") shiurim.push(entry);
      ...
      // A time with no keyword on the line itself — inherit meaning from the section header
  ...
  return {
    ...
    shiurim,
    other,
    // Kept so the website can always show *something* even if parsing misses a format,
    // and so we can improve the parser later without re-scraping.
    rawText: rawText.slice(0, 8000),
    api: apiPayloads.slice(0, 5),
