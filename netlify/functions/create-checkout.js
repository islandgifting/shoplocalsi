...
const SITE_URL = process.env.URL || process.env.DEPLOY_PRIME_URL || 'https://shoplocalsi.com';
const PLANS = {
  featured: { priceId: 'price_1TjqVIIDdMLYVh4oSlXr1DPM', label: 'Featured', successPath: '/success.html?plan=Featured' },
  featuredbusiness: { priceId: 'price_1TjqVIIDdMLYVh4oSlXr1DPM', label: 'Featured', successPath: '/success.html?plan=Featured' },
...
exports.handler = async (event) => {
  ...
  try {
    if (!process.env.STRIPE_SECRET_KEY) throw new Error('Missing STRIPE_SECRET_KEY in Netlify environment variables');
    const { plan } = JSON.parse(event.body || '{}');
    const key = String(plan || '').toLowerCase().replace(/[^a-z0-9]/g, '');
    const selected = PLANS[key];
    if (!selected) return { statusCode: 400, headers: corsHeaders(), body: JSON.stringify({ error: 'Unknown paid plan. Use featured, premium, or coupon.' }) };
