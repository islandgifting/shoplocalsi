...
// Add to every page with one line before </body>:
//   <script src="/js/minyan-bar.js" defer></script>
...
(function () {
  ...
  // Parse "7:15 AM" (or bare "7:15" using a section hint) into a Date today
  function parseTime(str, hintPM) {
    ...
    if (ap === "AM" && h === 12) h = 0;
    if (!ap) {
      // No AM/PM on the line — guess sensibly: shacharis is AM, mincha/maariv PM
      if (hintPM && h < 12) h += 12;
      else if (!hintPM && h <= 3) h += 12; // "1:45 mincha" style
    }
    var d = new Date();
  ...
  function nextFrom(entries, shuls) {
    ...
    entries.forEach(function (e) {
      ...
      // a line can hold several times ("6:30, 7:15, 8:00")
